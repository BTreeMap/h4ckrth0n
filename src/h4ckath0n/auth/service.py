...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if settings.first_user_is_admin:
        # Check for table emptiness using a fast scalar query, avoiding expensive O(N) count
...
async def register_user(
    ...
    *,
    ...
    # Fetch only ID to avoid expensive ORM parsing/hydration for an existence check
