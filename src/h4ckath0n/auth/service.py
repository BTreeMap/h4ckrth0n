...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if settings.first_user_is_admin:
        # Use limit(1) to avoid expensive aggregate operations and full table scans
...
async def register_user(
    ...
    *,
    ...
    # Use db.scalar(select(User.id)...) to avoid ORM parsing and hydration overhead
    existing_user_id = await db.scalar(select(User.id).filter(User.email == email))
