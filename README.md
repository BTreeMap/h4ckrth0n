...
The default authentication path uses passkeys (WebAuthn). The core flows involve passkey registration, login, adding devices, and passkey management (see API routes for endpoints).
...
`H4CKATH0N_PASSWORD_AUTH_ENABLED=true` (see API routes for endpoints).
