...
## 2026-08-19 - Use select(Model.id) and db.scalar for Existence and Single Entity Fetches

**Learning:** Using `select(func.count())` to check if a table is empty is inefficient because it counts all rows. Using `db.execute(...).scalars().first()` forces SQLAlchemy to allocate a Result object, parse the result, and potentially hydrate full models even if only an ID is needed.

**Action:** Use `db.scalar(select(Model.id).limit(1))` for fast emptiness checks. Use `db.scalar(select(Model))` to fetch a single object without `Result.scalars().first()` overhead, reducing memory allocations.
