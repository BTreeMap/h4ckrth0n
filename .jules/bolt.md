...
## 2024-07-28 - Optimize existence check in _is_bootstrap_admin
**Learning:** Using `func.count()` for database existence checks is an anti-pattern as it forces the database to count all matching rows.
**Action:** Always use `db.scalar(select(Model.id).limit(1))` and check for `None` to achieve a much faster O(1) existence check.
