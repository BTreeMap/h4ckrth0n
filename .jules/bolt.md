...
## 2026-03-22 - Avoid func.count() for existence checks

**Learning:** Using `select(func.count())` to check if a table is empty is an O(N) operation in some databases and generally slower than checking if a single record exists.
**Action:** Use `select(Model.id).limit(1)` and check if the result is `None` instead of `select(func.count())` when checking if a table is empty.
