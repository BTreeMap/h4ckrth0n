...
## 2024-05-15 - Optimize SQLAlchemy single record fetches
**Learning:** Using `await db.execute(select(Model)).scalars().first()` allocates an intermediate `ExecutionResult` and creates overhead, especially when checking existence or fetching a single column like an ID.
**Action:** Prefer `await db.scalar(select(...))` or `await db.scalar(select(Model.id).limit(1))` for single record or existence checks to improve memory efficiency and execution time.
