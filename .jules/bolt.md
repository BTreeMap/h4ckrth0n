...
## 2024-08-22 - Optimize boolean checks on table sizes
**Learning:** Found usage of `select(func.count())` just to check if records exist (e.g. if the user table is empty, or if more than 1 active passkey exists). This is an O(N) database query when it could be O(1) by fetching a single ID and checking for None using `.limit(1)`.
**Action:** Replace `select(func.count())` used for existence checks with `select(Model.id).limit(x)` and check if result is None (or check length).
