...
## 2026-03-24 - Use limit(1) instead of func.count() for Existence Checks
**Learning:** When checking if a database table is empty, using `select(func.count())` forces the database to count all matching rows, scanning the entire table. This is extremely inefficient for large tables.
**Action:** Use `await db.scalar(select(Model.id).limit(1))` and check if the result is `None`. This turns an O(N) scan into a much faster O(1) existence check.
