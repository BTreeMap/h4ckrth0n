...
## 2024-07-17 - Optimize Database Existence Checks
**Learning:** For database existence checks or single-field lookups with SQLAlchemy, using `db.scalar(select(Model.id))` is faster than `db.execute(select(Model)).scalars().first()` because it avoids the performance overhead of parsing and instantiating the full ORM object.
**Action:** Always prefer `db.scalar()` for existence checks or single-field lookups to minimize ORM overhead.
