...
## 2026-03-24 - Avoid O(N) func.count() when checking table emptiness
**Learning:** `select(func.count())` is an `O(N)` query on some databases (like PostgreSQL MVCC) and overkill when we just want to know if *any* records exist.
**Action:** Use `select(Model.id).limit(1)` and check for `None` instead of doing a full count to check for table emptiness.
