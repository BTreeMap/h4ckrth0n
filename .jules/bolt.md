...
## 2026-03-24 - Avoid full counts for existence checks

**Learning:** When checking if a database table is empty (or has any rows matching a condition), using `select(func.count())` is inefficient because it forces the database to count all matching rows.

**Action:** Use `select(Model.id).limit(1)` and check if the result is `None` via `db.scalar()`. This allows the database to return immediately upon finding the first row, turning an O(N) scan into an O(1) existence check.
