...
**Learning:** When checking if a database table is empty or has matching rows, `select(func.count())` forces the database to count all matching rows, which is an O(N) operation.
**Action:** Use `await db.scalar(select(Model.id).limit(1))` and check if the result is `None` instead of counting rows. This turns the operation into an O(1) existence check, saving CPU cycles and database bandwidth.
